...
|------|---------------------------|
| `characters.json` | **`sky-fc`:** seeded Sky FC party (**eight** characters, **six** slots each; §4.1 table). **`sky-sc`:** **fourteen** characters, **seven** slots each in [`games/sky-sc/characters.json`](games/sky-sc/characters.json) (seven core + seven guests). **`sky-tc`:** **sixteen** characters, **seven** slots each in [`games/sky-tc/characters.json`](games/sky-tc/characters.json) (Sky the 3rd party roster). **`zero`:** **nine** characters, **seven** slots each in [`games/zero/characters.json`](games/zero/characters.json) (Trails from Zero core party + guests; §4.1). **Array order:** checked-in **`sky-fc`**, **`sky-sc`**, **`sky-tc`**, and **`zero`** rosters are stored **alphabetically by `name`** (case-insensitive), matching `sort_characters_for_output` in [`scripts/build_trails_wiki_data.py`](scripts/build_trails_wiki_data.py) and the **`--resort-json-only`** path (§6.3). If you intentionally use story or party order, either skip resorting `characters.json` or restore order after `--resort-json-only`. |
...
**Sky SC roster (current):** [`games/sky-sc/characters.json`](games/sky-sc/characters.json) defines **fourteen** entries with **seven** orbment slots each (slot count is **not** six like FC). **JSON array order** is **alphabetical by `name`** (case-insensitive): Agate Crosner, **Anelace Elfead**, Estelle Bright, Josette Capua, **Joshua Bright** (SC uses this surname; FC §4.1 still lists **Joshua Astray**), **Julia Schwarz**, Kevin Graham, Kloe Rinz, **Kurt Nardin**, **Mueller Vander**, Olivier Lenheim, Scherazard Harvey, Tita Russell, Zin Vathek. **Seven core members** (`guest: false`): Agate, Estelle, Kloe, Olivier, Scherazard, Tita, Zin. **Seven guest members** (`guest: true`): **Anelace Elfead** (**wind** on slots **0** and **4**; **`line`** `0, 1, 1, 2, 3, 3, 4`), **Josette Capua** (**earth** on slots **0** and **2**; **`line`** `0, 1, 1, 1, 2, 2, 2`), **Joshua Bright** (**time** on slots **0** and **3**; **`line`** `0, 1, 1, 1, 1, 2, 2`), **Julia Schwarz** (**space** on slots **0** and **3**; **`line`** `0, 1, 1, 2, 2, 2, 2`), **Kevin Graham** (**time** slot **0**; **`line`** `0, 1, 1, 1, 1, 1, 2`), **Kurt Nardin** (**mirage** slot **0**; **`line`** `0, 1, 2, 2, 2, 2, 1`), **Mueller Vander** (**water** slot **0**; **`line`** `0, 1, 1, 2, 3, 3, 4`). Lines include **`4`** on some characters, so §5.5 line-tint classes through `quartz-slot-line-4` apply. **Post-seeding edits (authoritative in JSON):** **Tita Russell**—**space**-restricted slots on indices **0** and **3** (pattern `space`, `null`, `null`, `space`, …); **Scherazard Harvey**—slot **`line`** sequence **`0, 2, 2, 2, 2, 2, 1`**. There is no full per-slot table here—use the JSON as authoritative.
...
**Trails from Zero (`zero`) roster (current):** [`games/zero/characters.json`](games/zero/characters.json) lists **nine** entries with **seven** slots each. **Four core members** (`guest: false`): **Elie MacDowell** (**wind** on slots **0** and **4**; **`line`** `0, 1, 1, 1, 1, 1, 2`), **Lloyd Bannings** (all **`null`** elementals; **`line`** `0, 1, 1, 1, 2, 2, 2`), **Randy Orlando** (**fire** slot **0**; **`line`** `0, 1, 1, 1, 2, 2, 3`), **Tio Plato** (**water** on slots **0** and **4**; **`line`** `0, 1, 1, 1, 1, 1, 1`). **Five guest members** (`guest: true`): **Alex Dudley** (**time** slot **0**; **`line`** `0, 1, 1, 1, 1, 2, 2`), **Estelle Bright** (all **`null`** elementals; **`line`** `0, 1, 1, 1, 2, 2, 2`), **Joshua Bright** (**time** on slots **0** and **3**; **`line`** `0, 1, 1, 1, 1, 2, 2`), **Noel Seeker** (**earth** slot **0**; **`line`** `0, 1, 1, 1, 2, 3, 4`), **Yin** (**mirage** slot **0**; **`line`** `0, 1, 1, 1, 1, 2, 3`). **JSON array order** is **alphabetical by `name`** (§4, §6.3): Alex Dudley, Elie MacDowell, Estelle Bright, Joshua Bright, Lloyd Bannings, Noel Seeker, Randy Orlando, Tio Plato, Yin. Full slot data remains in the file.
...
- **Characters:** [`games/sky-sc/characters.json`](games/sky-sc/characters.json) is a **fourteen-character, seven-slot** SC orbment roster (seven core + seven guests; names in **alphabetical** JSON order—§4.1); it is **not** an FC roster copy. See §4.1.
...
- **`games/sky-sc/characters.json`:** Fourteen entries, seven `orbment` slots each; seven core (`guest: false`) + seven guests (`guest: true`): Anelace Elfead, Josette Capua, Joshua Bright, Julia Schwarz, Kevin Graham, Kurt Nardin, Mueller Vander. Matches §4.1 SC roster paragraph (not the FC eight-by-six table). Entries are **alphabetical by `name`** (§4, §6.3).
...
- **`games/zero/characters.json`:** Nine entries, seven slots each; §4.1 Trails from Zero roster paragraph. Four core members (`guest: false`) + five guest members (`guest: true`): Alex Dudley, Estelle Bright, Joshua Bright, Noel Seeker, Yin. Alphabetical **`name`** order.
...
3. **SC wiki + roster:** SC arts/quartz import; **`sky-sc`** CLI and `GAME_INPUTS`; dedicated **fourteen×seven** [`games/sky-sc/characters.json`](games/sky-sc/characters.json) — seven core (`guest: false`) plus seven guests (`guest: true`); alphabetical **`name`** order—§4.1, §6.3. FC vs SC quartz tables disambiguated by **seven** header cells (§6.3).
...
9. **Guest character support:** Added `guest` boolean field (default `false`) to all characters across all four games (§4.1 schema). Added "Show guest characters" checkbox to the UI (§5.1); `populateCharacterDropdown()` extracted from `onGameChange` to support filtering. Added five guest characters to **`zero`**: Alex Dudley, Estelle Bright, Joshua Bright, Noel Seeker, Yin (`guest: true`)—§4.1. Added seven guest characters to **`sky-sc`** (Royal Guard / temporary party members): Anelace Elfead, Julia Schwarz, Kurt Nardin, Mueller Vander as new entries; **Josette Capua**, **Joshua Bright**, and **Kevin Graham** reclassified from `guest: false` → `guest: true`—§4.1.
...
| SC vs FC characters | FC: eight×six (§4.1 table). SC: fourteen×seven in [`games/sky-sc/characters.json`](games/sky-sc/characters.json) — seven core (`guest: false`) + seven guests (`guest: true`): Anelace, Josette, Joshua, Julia, Kevin, Kurt, Mueller; hand-tuned orbments—§4.1, §8.1 item 5. |
| `sky-tc` characters | Sixteen×seven in [`games/sky-tc/characters.json`](games/sky-tc/characters.json); hand-tuned—§4.1, §8.1 item 5. |
| `zero` characters | Nine×seven in [`games/zero/characters.json`](games/zero/characters.json)—§4.1; four core (`guest: false`): Elie, Lloyd, Randy, Tio; five guests (`guest: true`): Alex Dudley, Estelle Bright, Joshua Bright, Noel Seeker, Yin. JSON **`name`** order alphabetical. |
